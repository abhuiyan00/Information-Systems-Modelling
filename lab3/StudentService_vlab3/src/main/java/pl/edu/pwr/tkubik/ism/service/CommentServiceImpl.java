...
import pl.edu.pwr.tkubik.ism.config.DataInitializer;
...
public class CommentServiceImpl implements CommentService {
    ...
    @Override
    public Comment addComment(UUID buildId, String content) {
        Optional<BuildEntity> buildOpt = buildRepository.findById(buildId);
        if (buildOpt.isEmpty()) return null;

        // Always link the comment to the seeded system user so
        // author_id is populated instead of being null.
        UserEntity author = userRepository
                .findById(DataInitializer.SYSTEM_USER_ID)
                .orElse(null);
